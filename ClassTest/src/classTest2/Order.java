...
public class Order {
    ...
    double calculateDiscount() {
        double totalAmount = calculateTotal(); // 총 금액
        if (customer instanceof PrermiumCustomer) {
            PrermiumCustomer premiumCustomer = (PrermiumCustomer) customer;
            double discountRate = premiumCustomer.getDiscountRate(); // 할인율
            double discountAmount = totalAmount * discountRate; // 할인 금액 계산
            double finalAmount = totalAmount - discountAmount; // 최종 금액
            double calDiscount = Math.ceil(discountAmount); // 할인 금액 올림
            return calDiscount;
        }
        return 0.0;
    }
    ...
    void printOrderSummary() {
        ...
        if (customer instanceof PrermiumCustomer) {
            System.out.println("     => " + calculateDiscount() + " [Discount]");
        }
